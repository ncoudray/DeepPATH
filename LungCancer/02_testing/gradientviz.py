...
def outer_grad_inception(labels, batch_size=1, learningrate=0.1, weight_decay=0.1, num_classes =1):
    ...
    sparse_labels = tf.reshape(y, [batch_size, 1])
    ...
    dense_labels = tf.sparse_to_dense(concated,
                                      ...
                                      1.0, 0.0)
    logits, auxiliary_logits, endpoints, net2048 = inference(x, 1)
    ...
    with tf.get_default_graph() and tf.Session(config=tf.ConfigProto(allow_soft_placement=True))as sess:
        ...
        # loss_grad = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='tower_0/CrossEntropyLoss/value/avg')[0]
        ...
        print "sess var: ", sess.run(var_grad, feed_dict={x: image, y: 1})


        for i in range(10):
            x_grad = sess.run(var_grad, feed_dict={x: image, y: 1})
            print x_grad
